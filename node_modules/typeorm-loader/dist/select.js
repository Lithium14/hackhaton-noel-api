...
function select(model, selection, connection, qb, alias, history) {
    ...
    if (selection && selection.children) {
        ...
        fields.forEach(function (field) { return qb = qb.addSelect(alias + "." + field.propertyName, alias + "_" + field.propertyName); });
        ...
        relations.forEach(function (relation) {
            if (relation.propertyName in selection.children) {
                ...
                if (relation.inverseRelation) {
                    qb = qb.addFrom(relation.inverseRelation.entityMetadata.targetName, relation.inverseRelation.entityMetadata.targetName);
                    qb = qb.leftJoin(alias + '.' + relation.propertyName, childAlias);
                    qb = select(relation.inverseEntityMetadata.target, selection.children[relation.propertyName], connection, qb, childAlias);
                }
            }
        });
    }
